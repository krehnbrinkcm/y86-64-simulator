...
bool DecodeStage::doClockLow(PipeReg ** pregs) {
	...
	uint64_t valb = dreg->get(D_VALB);
	uint64_t valp = dreg->get(D_VALP);

	setEInput(ereg, stat, icode, ifun, 0, 0, valb, );	
	return false;
} 
...
void DecodeStage::setEInput(PipeReg * ereg, uint64_t e_stat, uint64_t e_icode, uint64_t e_ifun, uint64_t e_valc, uint64_t e_vala,
